...
public class ContinuousServoGroup_Sample extends OpMode {
    ...
    public void init() {
        ...
        servos = new ContinuousServoGroup(new ContinuousServo(hardwareMap, "contServo1", false),
                new ContinuousServo(hardwareMap, "const2", true));
    ...
    public void loop() {
        ...
        if (driver.onPress(Button.A))
            servos.setTargetPower(powers[0]); //Set servo powers to 0
        else if (driver.onPress(Button.B))
            servos.setTargetPower(powers[1]); //Set servo powers to 0.3
        else if (driver.onPress(Button.X))
            servos.setTargetPower(powers[2]); //Set servo powers to 0.7
        else if (driver.onPress(Button.Y))
            servos.setTargetPower(powers[3]); //Set servo powers to 1
